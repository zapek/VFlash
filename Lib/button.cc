...
// $Id: button.cc,v 1.1.1.1 2002/10/07 12:52:19 zapek Exp $

#include "swf.h"
#include "../debug.h"

#ifdef RCSID
static char *rcsid = "$Id: button.cc,v 1.1.1.1 2002/10/07 12:52:19 zapek Exp $";
#endif
...
Button::getActionFromTransition(ButtonState cur, ButtonState old)
{
	...
	for (cond = conditionList; cond; cond = cond->next) {
		...
		/* Ollicode (tm) */
			ActionRecord *tr = cond->actions;
			while (tr)
			{
				ActionRecord *clone = new ActionRecord;
				*clone = *tr;
				addActionRecord(clone);
				tr = tr->next;
			}
		}
	}
	...
	if (actionRecords)
	{
		ActionRecord *res = actionRecords;
		actionRecords = 0;
		return res;
	}
	else
