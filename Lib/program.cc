...
// $Id: program.cc,v 1.2 2004/04/15 19:33:09 zapek Exp $
...
#ifdef RCSID
static char *rcsid = "$Id: program.cc,v 1.2 2004/04/15 19:33:09 zapek Exp $";
#endif
...
Program::runFrame(GraphicDevice *gd, SoundMixer *sm, long f, long action)
{
	...
	for(ctrl = frames[f].controls; ctrl; ctrl = ctrl->next)
	{
		...
		// Actions
			...
			case ctrlStartSound:
				if (action && sm) {
					sm->startSound( (Sound *)ctrl->character, 0);//ctrl->loopcnt );
				}
				break;
			case ctrlStopSound:
				if (action && sm) {
...
void setFlashTimer(struct unixtimeval *tv, int time_ms)
{
	if (time_ms == -1) {
		tv->tv_sec = -1;
	} else {
#ifdef AMIGA
	/*
	 * Here we use smarter timers instead of
	 * the gettimeofday() which is not only ugly but
	 * also breaks when the time is set backwards on the box
	 */
	tv->tv_sec = 0;
...
int checkFlashTimer(struct unixtimeval *tv)
{
#ifndef AMIGA
	struct unixtimeval now;
#endif
	...
	while(action)
	{
		...
		switch (action->action)
		{
...
#endif
								if (sm) {
									sm->startSound(action->sound, 0); /* XXX */
								}
				status |= WAKEUP;
				break;
			case ActionRefresh:
			...
			case ActionSetTarget:
				...
				/* Ollicode (tm) */
				{
					Control *ct = frames[currentFrame].controls;

					//DB(("ct == %lx\n", ct)); /* bug here, control is near $ffffffff and ct->name gets 2 */

					while (ct)
					{
						if (ct->name && !strcmp(ct->name, &action->target[1]))
						{
							Sprite *sp = (Sprite*)ct->character;
							if (sp)
							{
								int tempstatus;
								ActionRecord tempaction = *action->next;
								tempaction.next = 0;
								tempstatus = sp->getProgram()->doAction(gd, &tempaction, sm);
								if (tempstatus & GOTO)
								{
									sp->getProgram()->gotoFrame(gd, sp->getProgram()->nextFrame);
								}
								break;
							}
						}
